...
class MixUp(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
 	...
 	if (mix_len%2)!=0:
        ...
        data = data[:mix_len,:,:,:]
  #      print mix_len
 #       print unmix_data.shape,data.shape
...
#	print y.shape
 ...
 #       label[label==0] = -1
        ...
        y1 = label[:length,:]
        y2 = label[length:,:]
#        print y1.shape,y2.shape,data1.shape,data2.shape
...
#        print unmix_label.shape,unmix_data.shape,data.shape,label.shape
    ...
    def app_backward(self, req, out_grad, in_data, out_data, in_grad, aux):
        ...
        data = in_data[0].asnumpy()  # shape=(b,d)
        y = in_data[1].asnumpy().astype('int')
        length = data.shape[0] / 2
...
#        print y1.shape,y2.shape,data1.shape,data2.shape
        ...
        out_g = mx.nd.SliceChannel(out_grad[0],axis=0, num_outputs=2) 
        data1.attach_grad()
	data2.attach_grad()
	with autograd.record():
		data= self.dif_mix(data1,data2,self.data_lamdba1)
        data.backward(out_g[0])
